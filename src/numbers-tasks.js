...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const magnitudeV = Math.sqrt(x1 ** 2 + y1 ** 2);
  const magnitudeW = Math.sqrt(x2 ** 2 + y2 ** 2);

  const cosineTheta = dotProduct / (magnitudeV * magnitudeW);
  const theta = Math.acos(Math.min(1, Math.max(-1, cosineTheta)));
